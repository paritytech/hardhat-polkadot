...
describe("constants", () => {
    ...
    it("defines task names with proper hierarchy", () => {
        expect(TASK_COMPILE_SOLIDITY_GET_RESOLC_BUILD).toMatch(/^compile:/)
        expect(TASK_COMPILE_SOLIDITY_COMPILE_RESOLC).toMatch(/^compile:/)
        expect(TASK_COMPILE_SOLIDITY_RUN_RESOLC).toMatch(/^compile:/)
    })
})
